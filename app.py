...
def index():
    ...
    return render_template(
        "index.html", stocks=user_stocks, cash=user_cash, grand_total=grand_total
    )
...
def buy():
    ...
    if request.method == "POST":
        ...
        db.execute(
            "INSERT INTO purchases (user_id, symbol, shares, price, timestamp) VALUES (?, ?, ?, ?, datetime('now'))",
            session["user_id"],
            quote_info["symbol"],
            shares,
            quote_info["price"],
...
def history():
    ...
    transactions = db.execute(
        "SELECT symbol, shares, price, timestamp FROM purchases WHERE user_id = ? ORDER BY timestamp DESC",
        session["user_id"],
    )
...
def quote():
    ...
    if request.method == "POST":
        ...
        if quote_info is None:
            return apology("Provice valid symbol!", 400)
...
def sell():
    ...
    if request.method == "POST":
        # Checks user's input
        if not request.form.get("symbol"):
            return apology("Select stock symbol!", 400)
        elif not request.form.get("shares"):
            return apology("Provide number of shares!", 400)
        ...
        try:
            ...
            return apology("Number of shares have to be positive!", 400)
        ...
        if not user_shares or shares > user_shares[0]["total_shares"]:
            return apology("You don't own that amount of shares of this stock!", 400)
